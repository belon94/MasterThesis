###############################################################################
#   DCC-GARCH Bayesian Neural Network with Normal Distribution 
#   - MAP Estimation (comparison with t-distribution version) - FIXED VERSION  
###############################################################################
...
# Output folder
const OUTPUT_FOLDER = "map_normal_results"
...
# ✅ FIXED FUNCTION: Corrected VaR calculation
function calculate_portfolio_var(portfolio_weights, forecast_mean, cov_matrix; confidence_levels=(0.01, 0.05, 0.10))
    ...
    σp = sqrt(max(Σp, 0.0))  # Standard deviation for Normal distribution
    
    var_results = Dict{Float64, Float64}()
    for α in confidence_levels
        z_quantile = quantile(Normal(0, 1), α)  # This is NEGATIVE for α < 0.5 (e.g., -2.326 for α=0.01)
        # ✅ CORRECTED: VaR(α) = -(μp + z_α * σp) = -μp - z_α * σp
        # Since z_α is negative, this becomes: -μp - (negative value) = -μp + |z_α| * σp
        var_results[α] = -μp - z_quantile * σp  # ✅ FIXED: Added negative sign to z_quantile
    end
...
function plot_enhanced_var_analysis(portfolio_returns::Vector{Float64}, daily_portfolio_vars::Vector{Vector{Float64}}, var_backtest_results::Dict{Float32, VaRResults}, asset_names::Vector{String}, model_name::String)
    ...
    try
        p1 = plot(title="Portfolio VaR vs Actual Returns ($model_name - Normal)", xlabel="Day", ylabel="Return/VaR (% points)", size=(1000, 400))
        ...
        p2 = bar(title="VaR Backtesting Results ($model_name - Normal)", xlabel="Confidence Level", ylabel="Test Statistic", size=(1000, 400))
        ...
        p3 = histogram(portfolio_returns, bins=30, alpha=0.7, title="Return Distribution with VaR Levels ($model_name - Normal)", xlabel="Portfolio Returns", ylabel="Frequency", label="Return Distribution", color=:lightblue, size=(1000, 400))
...
function compute_dcc_likelihood(outs, y, a, b, N, Tsteps, prior)
    ...
    for t in 1:Tsteps
        ...
        logl += -0.5 * (N * log_2pi + 2 * sum(log, diag(L)) + quad)
...
function train_model_map(model_type::Symbol, X_train, Y_train, X_val, Y_val, N::Int; 
        hidden_size::Int=32, prior_std::Float32=0.15f0)

    println("Training $model_type model with $N assets (MAP estimation, Normal distribution)...")
...
function rolling_covariance_forecast_map(bnn_train, θmap, rolling_data, model_type::Symbol, N::Int)
    println("Performing MAP-based rolling window forecasting with portfolio analysis (Normal distribution)...")
...
function evaluate_model_map(bnn_train, bnn_val, bnn_test_forecasts, θmap, model_name::String)
    N = bnn_train.like.N
    net = bnn_train.like.nc(θmap[1:end-2])
...
function print_map_results_summary(results, n_assets)
    println("COMPREHENSIVE RESULTS SUMMARY (MAP, $n_assets Assets, Normal distribution)")
    println("\n┌─────────────────────┬──────────────┬─────────────────────────────────────────────────┐")
...
function save_var_backtest_to_csv(portfolio_results::Dict, n_assets::Int, filename_prefix::String="var_backtest_map_normal")
    all_data = []
...
function save_model_performance_summary(results::Dict, n_assets::Int)
    all_data = []
...
# ══════════════════════════════════════════════════════════════════════════════
#                SAVE PORTFOLIO VAR VS RETURNS DATA (MAP VERSION)
# ══════════════════════════════════════════════════════════════════════════════

function save_portfolio_var_returns(portfolio_results::Dict, n_assets::Int)
    println("Saving Portfolio VaR vs Returns data (MAP Normal)...")
        ...
        if haskey(result_dict, "forecasts")
            ...
            if haskey(forecasts, "portfolio_returns") && haskey(forecasts, "portfolio_var_1")
                ...
                var_df = DataFrame(
                    ...
                    Distribution = fill("Normal", n_obs),
                    Model = fill(model_name, n_obs)
                )
                
                csv_path = joinpath(OUTPUT_FOLDER, "portfolio_var_returns_map_normal_$(model_name_short)_$(n_assets)_assets.csv")
...
function comprehensive_dcc_analysis_map(data_path::String, n_assets::Int=30)
    println("COMPREHENSIVE DCC-GARCH ANALYSIS (MAP-BASED, Normal distribution)")
    ...
    for (model_type, X_train, Y_train, X_val, Y_val, rolling_data, model_name) in models_to_run
        println("\n" * "="^60)
        println("TRAINING AND EVALUATING $model_name (MAP, Normal)")
        println("="^60)
        
        bnn_train, bnn_val, θmap, _ = train_model_map(
            model_type, X_train, Y_train, X_val, Y_val, n_assets; hidden_size=hidden_size)

        portfolio_forecasts = rolling_covariance_forecast_map(
            bnn_train, θmap, rolling_data, model_type, n_assets)
        ...
        result = evaluate_model_map(bnn_train, bnn_val, portfolio_forecasts, θmap, model_name)
        ...
        if haskey(portfolio_forecasts, "portfolio_returns") && haskey(portfolio_forecasts, "var_backtest_results")
            ...
            if var_plot !== nothing
                display(var_plot)
                plot_filename = joinpath(OUTPUT_FOLDER, "map_normal_var_analysis_$(model_type)_$(n_assets)_assets.png")
        ...
        if haskey(portfolio_forecasts, "portfolio_volatility")
            mvp_vol_plot = plot(1:length(portfolio_forecasts["portfolio_volatility"]), 
                                ...
                                title="MVP Volatility Evolution (MAP Normal) - $model_name ($n_assets Assets)", 
                                xlabel="Day", ylabel="Portfolio Volatility (% points)", 
                                linewidth=2, color=:purple, label="MVP Volatility",
                                size=(1000, 400))
            display(mvp_vol_plot)
            vol_filename = joinpath(OUTPUT_FOLDER, "map_normal_mvp_evolution_$(model_type)_$(n_assets)_assets.png")
            savefig(mvp_vol_plot, vol_filename)
            println("  MVP evolution plot saved: $vol_filename")
        end
    end

    print_map_results_summary(results, n_assets)
    print_portfolio_comparison(portfolio_results, n_assets) 
    print_var_backtesting_summary(portfolio_results, n_assets)
    
    # Save all results to CSV files
    save_var_backtest_to_csv(portfolio_results, n_assets, "var_backtest_map_normal")
    save_model_performance_summary(results, n_assets)
    save_portfolio_weights(portfolio_results, n_assets)
    save_portfolio_var_returns(portfolio_results, n_assets)
...
for n_assets in asset_counts
    println("RUNNING MAP-BASED ANALYSIS FOR $n_assets ASSETS (Normal distribution)")
    
    results, portfolio_results = comprehensive_dcc_analysis_map(data_path, n_assets)
    all_results_map[n_assets] = (results, portfolio_results)
    
    println("Analysis complete for $n_assets assets! Results saved and plots generated.")
end
