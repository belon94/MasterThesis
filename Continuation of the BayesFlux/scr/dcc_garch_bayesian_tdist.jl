...
function calculate_portfolio_var(portfolio_weights, forecast_mean, cov_matrix, nu::Float64; confidence_levels=(0.01, 0.05, 0.10))
    ...
    σp = sqrt(max(Σp, 0.0)) * sqrt(nu / (nu - 2))
    
    var_results = Dict{Float64, Float64}()
    for α in confidence_levels
        t_quantile = quantile(TDist(nu), α)
        var_results[α] = -μp + t_quantile * σp
    end
...
# ══════════════════════════════════════════════════════════════════════════════
#                      ROLLING PORTFOLIO FORECASTING WITH VAR BACKTESTING
# ══════════════════════════════════════════════════════════════════════════════
...
function rolling_covariance_forecast_with_portfolio_analysis(bnn_train, chain2d, rolling_data, model_type::Symbol, N::Int, nu::Float64)
    println("Performing rolling window forecasting with portfolio analysis and VaR backtesting (t-distribution, ν=$nu)...")
    ...
    for (t_idx, (X_t, Y_t, _)) in enumerate(rolling_data)
        ...
        if valid > 0
            ...
            error("No valid predictions available for time step $t_idx")
        end
...
function comprehensive_dcc_analysis_bayesian(data_path::String, n_assets::Int=30, nu::Float64=5.0)
    println("COMPREHENSIVE DCC-GARCH ANALYSIS (Bayesian, t-distribution with ν=$nu)")
