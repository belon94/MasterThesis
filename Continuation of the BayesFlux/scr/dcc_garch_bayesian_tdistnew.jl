...
const OUTPUT_FOLDER = "dcc_tdist_results"
...
function plot_enhanced_var_analysis(portfolio_returns::Vector{Float64}, daily_portfolio_vars::Vector{Vector{Float64}}, var_backtest_results::Dict{Float32, VaRResults}, asset_names::Vector{String}, model_name::String)
    ...
    try
        p1 = plot(title="Portfolio VaR vs Actual Returns ($model_name - BNN t-dist)", xlabel="Day", ylabel="Return/VaR", size=(1000, 400))
        ...
        p2 = bar(title="VaR Backtesting Results ($model_name - BNN t-dist)", xlabel="Confidence Level", ylabel="Test Statistic", size=(1000, 400))
        ...
        p3 = histogram(portfolio_returns, bins=30, alpha=0.7, title="Return Distribution with VaR Levels ($model_name - BNN t-dist)", xlabel="Portfolio Returns", ylabel="Frequency", label="Return Distribution", color=:lightblue, size=(1000, 400))
...
function train_model_with_validation(model_type::Symbol, X_train, Y_train, X_val, Y_val, N::Int; 
        ...
        mcmc_samples::Int=10000, chains::Int=4, nu::Float32=5.0f0)

    println("Training $model_type model with $N assets (Bayesian, t-distribution with ν=$nu)...")
    if model_type == :rnn
...
function rolling_covariance_forecast_with_portfolio_analysis(bnn_train, chain2d, rolling_data, model_type::Symbol, N::Int, nu::Float64)
    println("Performing FULLY BAYESIAN rolling window forecasting with portfolio analysis and VaR backtesting (t-distribution, ν=$nu)...")
...
function evaluate_model_comprehensive(bnn_train, bnn_val, chain2d, ch_raw, portfolio_forecasts, model_name::String, nu::Float64)
    N = bnn_train.like.N
    ...
    test_rmse = sqrt(mean((vec(portfolio_forecasts["actuals"]) .- vec(portfolio_forecasts["means"])).^2))
...
function evaluate_portfolio_performance(forecasts::Dict{String, Any}, model_name::String)
    ...
    finite_var_1  = filter(isfinite, Float64.(forecasts["portfolio_var_1"]))
    finite_var_5  = filter(isfinite, Float64.(forecasts["portfolio_var_5"]))
    finite_var_10 = filter(isfinite, Float64.(forecasts["portfolio_var_10"]))
    ...
    return Dict(
        ...
        "avg_var_1"  => length(finite_var_1)  > 0 ? mean(finite_var_1)  : NaN,
        "avg_var_5"  => length(finite_var_5)  > 0 ? mean(finite_var_5)  : NaN,
        "avg_var_10" => length(finite_var_10) > 0 ? mean(finite_var_10) : NaN
    )
end

function print_comprehensive_results_summary(results, n_assets, nu)
    println("\n" * "="^80)
    println("COMPREHENSIVE RESULTS SUMMARY (Bayesian, $n_assets Assets, t-distribution ν=$nu)")
    println("="^80)
    println("\n┌─────────────────────┬─────────┬──────────────┬─────────────────────────────────────────────────┐")
    ...
    for (model_type, name) in [(:rnn, "Sequential RNN"), (:lstm, "Sequential LSTM")]
        if haskey(results, model_type)
            ...
            α, β = r["dcc_params"]
            println("│ $(rpad(name, 19)) │ $(lpad(string(round(r["r_hat"], digits=3)), 7)) │ $(lpad(string(round(α, digits=3)), 4)), $(lpad(string(round(β, digits=3)), 4)) │ $(lpad(string(round(r["train_rmse"], digits=6)), 7)) / $(lpad(string(round(r["validation_rmse"], digits=6)), 7)) / $(lpad(string(round(r["test_rmse"], digits=6)), 7)) │")
        end
    end
    println("└─────────────────────┴─────────┴──────────────┴─────────────────────────────────────────────────┘")
end
...
function save_var_backtest_to_csv(portfolio_results::Dict, n_assets::Int, nu::Float64, filename_prefix::String="var_backtest_bayesian_tdist")
    all_data = []
...
function save_model_performance_summary(results::Dict, n_assets::Int, nu::Float64)
    ...
    if !isempty(all_data)
        ...
        CSV.write(csv_filename, df)
        println("Model performance summary saved to: $csv_filename")
    end
end
        ...
        if haskey(portfolio_results, model_type)
            ...
            if haskey(result_dict, "forecasts")
                ...
                if haskey(forecasts, "mv_weights")
                    ...
                    println("Portfolio weights saved to: $csv_filename")
                end
            end
        end
    end
...
# ══════════════════════════════════════════════════════════════════════════════
#                 NEW FUNCTION: SAVE PORTFOLIO VAR VS RETURNS DATA 
# ══════════════════════════════════════════════════════════════════════════════
    ...
    for (model_type, result_dict) in portfolio_results
        if haskey(result_dict, "forecasts")
            ...
            if haskey(forecasts, "portfolio_returns") && haskey(forecasts, "portfolio_var_1")
                ...
                csv_path = joinpath(OUTPUT_FOLDER, "portfolio_var_returns_tdist_$(model_name_short)_$(n_assets)_assets.csv")
...
# NEW: mirrors ThesisMIMO portfolio_performance_[N]_assets.csv
function save_portfolio_performance_csv(portfolio_results::Dict, n_assets::Int, nu::Float64)
    perf_df = DataFrame(
        Model = String[],
        Mean_Return = Float64[],
        Realized_Volatility = Float64[],
        Sharpe_Ratio = Float64[],
        Avg_Predicted_Volatility = Float64[],
        Avg_VaR_1 = Float64[],
        Avg_VaR_5 = Float64[],
        Avg_VaR_10 = Float64[],
        N_Assets = Int[],
    ...
    for (model_type, name) in [(:rnn, "Sequential RNN"), (:lstm, "Sequential LSTM")]
        if haskey(portfolio_results, model_type)
            ...
            push!(perf_df, (
                perf["model_name"],
                Float64(perf["mean_return"]),
                Float64(perf["volatility"]),
                Float64(perf["sharpe_ratio"]),
                Float64(perf["avg_predicted_volatility"]),
                Float64(perf["avg_var_1"]),
                Float64(perf["avg_var_5"]),
                Float64(perf["avg_var_10"]),
                n_assets,
                nu
            ))
        end
    ...
    println("✓ Portfolio performance saved to: $csv_path")
end
...
function comprehensive_dcc_analysis_bayesian(data_path::String, n_assets::Int=30, nu::Float64=5.0)
    println("COMPREHENSIVE DCC-GARCH ANALYSIS (FULLY BAYESIAN, t-distribution with ν=$nu)")
    println("Proper uncertainty propagation through DCC parameters")
    ...
    for (model_type, X_train, Y_train, X_val, Y_val, rolling_data, model_name) in models_to_run
        println("\n" * "="^60)
        println("TRAINING AND EVALUATING $model_name WITH PORTFOLIO ANALYSIS & VAR BACKTESTING (Bayesian t-dist)")
        println("="^60)
        bnn_train, bnn_val, chain2d, ch_raw, net, val_loglik = train_model_with_validation(
            model_type, X_train, Y_train, X_val, Y_val, n_assets; hidden_size=hidden_size, mcmc_samples=10000, chains=4, nu=Float32(nu))
        ...
        portfolio_forecasts = rolling_covariance_forecast_with_portfolio_analysis(
            bnn_train, chain2d, rolling_data, model_type, n_assets, nu)
        ...
        result = evaluate_model_comprehensive(bnn_train, bnn_val, chain2d, ch_raw, portfolio_forecasts, model_name, nu)
        ...
        println("  Training RMSE: $(round(result["train_rmse"], digits=6))")
        println("  Validation RMSE: $(round(result["validation_rmse"], digits=6))")
        println("  Test RMSE: $(round(result["test_rmse"], digits=6))")
        ...
        if haskey(portfolio_forecasts, "portfolio_returns") && haskey(portfolio_forecasts, "var_backtest_results")
            ...
            if var_plot !== nothing
                display(var_plot)
                plot_filename = joinpath(OUTPUT_FOLDER, "bayesian_tdist_var_analysis_$(model_type)_$(n_assets)_assets.png")
        ...
        if haskey(portfolio_forecasts, "portfolio_volatility")
            mvp_vol_plot = plot(1:length(portfolio_forecasts["portfolio_volatility"]), 
                              ...
                              title="MVP Volatility Evolution - $model_name ($n_assets Assets, Bayesian t-dist)", 
                              xlabel="Day", ylabel="Portfolio Volatility", 
                              linewidth=2, color=:purple, label="MVP Volatility",
                              size=(1000, 400))
            display(mvp_vol_plot)
            vol_filename = joinpath(OUTPUT_FOLDER, "bayesian_tdist_mvp_evolution_$(model_type)_$(n_assets)_assets.png")
            savefig(mvp_vol_plot, vol_filename)
            println("  MVP evolution plot saved: $vol_filename")
        end
    end

    print_comprehensive_results_summary(results, n_assets, nu)
    print_portfolio_comparison(portfolio_results, n_assets)
    print_var_backtesting_summary(portfolio_results, n_assets)
    
    # Save all results to CSV files
    save_var_backtest_to_csv(portfolio_results, n_assets, nu, "var_backtest_bayesian_tdist")
    save_model_performance_summary(results, n_assets, nu)
    save_portfolio_weights(portfolio_results, n_assets)
    save_portfolio_var_returns(portfolio_results, n_assets, nu)  # NEW: Added this function call
    save_portfolio_performance_csv(portfolio_results, n_assets, nu)  # NEW: portfolio_performance_[N]_assets.csv
...
for n_assets in asset_counts
    println("RUNNING BAYESIAN ANALYSIS FOR $n_assets ASSETS (t-distribution, ν=$nu_value)")
    
    results, portfolio_results = comprehensive_dcc_analysis_bayesian(data_path, n_assets, nu_value)
    all_results[n_assets] = (results, portfolio_results)
    
    println("\nAnalysis complete for $n_assets assets! Results saved and plots generated.")
end

println("ALL BAYESIAN ANALYSES COMPLETE (t-distribution)!")
